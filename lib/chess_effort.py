...
def classify(board: chess.Board, novel=None) -> tuple[str, list[str]]:
    """The effort a position deserves: (level, reasons-that-fired). LOW with
    no reasons is the quiet default; any reason at all means HIGH — a cheap
    alarm that rings falsely now and then costs one deliberate turn, where an
    alarm that stays quiet falsely can cost the game. Pure arithmetic on the
    board; `novel` is the store's verdict (novelty()), passed in so this
    function stays free of I/O and deterministic under test."""
    us = board.turn
    reasons = []
