...
// time, each exactly once — the phone's own playback discipline scaled to
// the table. A failed index is dead for good and never refetched.
let clipQueue = [];        // {game, n} awaiting their turn to sound
let clipPlaying = false;   // a clip is being fetched or sounding right now
let clipDead = {};         // "game#n" -> true: failed for good
...
function clipFailed(c, why) {
	// Dead for good (rule 24e; phone.md rule 44a is the pedigree): never
	// refetched, never retried — the text keeps the words. The browser's
	// built-in narrator is NEVER reached for: a generic voice wearing her
	// words is worse than silence, so the console line is the witness.
	clipDead[clipKey(c.game, c.n)] = true;
...
function enqueueClip(game, n) {
	...
	// the first shipped shape ("one clip per batch is plenty") did.
	if (clipDead[clipKey(game, n)]) return;
	clipQueue.push({game, n});
	if (!clipPlaying) playNextClip();
}

function playNextClip() {
	const c = clipQueue.shift();
	if (!c) { clipPlaying = false; return; }
	if (!chatSpeakOn()) { clipPlaying = false; clipQueue = []; return; }
	clipPlaying = true;
	// The bridge synthesizes the RECORDED message through the same piper
	// pipeline every other voice of hers comes from (rule 24e). A clip
	// that cannot be fetched, decoded, or started keeps the text, says so
	// in the console, and the queue moves on — one dead clip never parks
	// the thread's voice; a clip already sounding is never cut.
	fetch('/chat/audio?game=' + encodeURIComponent(c.game || '')
		...
		.then(b => new Promise(done => {
			...
			const settle = why => {
				...
				if (why) clipFailed(c, why);
				done();
			};
			// The never-started give-up (phone.md rule 44b's shape): a
			// clip showing no sound within the grace is given up on and
			// the queue advances; one that started is never cut by it.
			const watchdog = setTimeout(() => {
				if (started) return;
				try { clip.pause(); } catch (e) { /* already dead */ }
				settle('never started');
			}, CLIP_START_GRACE_MS);
			clip.addEventListener('playing', () => { started = true; });
			clip.addEventListener('timeupdate', () => {
				if (clip.currentTime > 0) started = true;
			});
			clip.onended = () => settle(null);
			clip.onerror = () => settle('media error');
			clip.play().catch(e =>
				settle((e && e.message) || 'playback refused'));
		}))
		.catch(e => clipFailed(c, (e && e.message) || e))
		.then(() => { clipPlaying = false; playNextClip(); });
}
