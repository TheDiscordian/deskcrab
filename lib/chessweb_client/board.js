...
// Reconnection (specs/chessweb.md rule 25): from the user's Connect on, the
// page owns the wire. A drop schedules a redial on an exponential backoff —
// one second doubling to the cap, jittered so a herd of tabs cannot knock in
// step — paused while the page is hidden; `online` and a return to
// visibility redial at once with the backoff reset. A redial that lands
// resumes the old role uninvited, and rule 3's join sync (Team + replay) is
// the resync: the store repaints the board, never the page's own memory.
const RECONN_BASE_MS = 1000, RECONN_CAP_MS = 30000;
const REJOIN_TRIES = 3;    // the dead-seat probe may need one more ask
let wantLive = false;      // Connect was clicked; keep this page on the wire
let reconnTimer = null;    // the pending redial
let reconnDelay = 0;       // the current backoff step (0 = fresh)
let reconnRole = null;     // 'player' | 'watcher' — the role to resume
let rejoinsLeft = 0;       // re-asks left for a seat the probe frees late
let rejoinWait = false;    // an auto re-Join is out, no Player back yet
...
let chatPoll = null;
let chatBusy = false;
// The clip queue (rule 24e): her arriving messages sound in order, one at a
// time, each exactly once — the phone's own playback discipline, ONE shared
// implementation now (phone.md rule 44c): lib/browser_voice_queue.js, the
// same file the phone server serves, loaded by index.html beside this one.
// A failed index is dead for good and never refetched; this file keeps the
// POLICY only — the fetch, the console witness, the speak-toggle gate.
const CLIP_START_GRACE_MS = 10000;  // the never-started give-up bound
let playerLabel = null;    // /chat's player — the LOADED game's own label
let assistantName = 'The assistant';
// The phone conversation's presentation (THE SHIPPED CLIENT bullet): turns
// rather than a labeled transcript, the dimmed history seed, and the pending
// echo of a send until the record's copy adopts it.
let chatTurn = null;       // the open turn awaiting her answer {el, answered, seed}
let chatSeed = null;       // the history seed section, while a backlog renders
let chatEchoes = [];       // locally echoed sends awaiting the record's copy
...
async function fetchState() {
	...
	gameId = st.game;
	// Rule 23e: the ratings shown belong to the loaded game's speed pool
	// and sitter — refetched only when that identity changes.
	const eloNow = st.game + '|'
		+ ((st.time_control && st.time_control.speed) || 'untimed') + '|'
		+ (st.player || '');
	if (eloNow !== eloKey) {
		eloKey = eloNow;
		eloView = {
			speed: (st.time_control && st.time_control.speed) || 'untimed',
			player: st.player || null,
			humanSide: st.human_side,
		};
		fetchElo();
	}
...
// ---------------------------------------------------------------- elo
// Rule 23e: each colour's rating beside its graveyard title, from GET /elo
// — the server-computed per-speed pools — resolved here for the loaded
// game's speed and sitter. A missing pool or bucket shows nothing: Elo
// needs a finished game behind it, and an empty corner beats an invented
// 1200.
let eloPools = null;
let eloView = null;
let eloKey = null;
...
function renderElo() {
	const w = $('gy-w-elo'), b = $('gy-b-elo');
	if (!w || !b) return;
	w.hidden = b.hidden = true;
	if (!eloPools || !eloView) return;
	const pool = eloPools[eloView.speed];
	if (!pool) return;
	const her = pool.her ? pool.her.rating : null;
	const key = slugName(eloView.player || 'browser');
	const bucket = (pool.players || [])
		.find(p => slugName(p.player) === key);
	const sitter = bucket ? bucket.rating : null;
	const white = eloView.humanSide === 'white' ? sitter : her;
	const black = eloView.humanSide === 'white' ? her : sitter;
	if (white != null) { w.textContent = white; w.hidden = false; }
	if (black != null) { b.textContent = black; b.hidden = false; }
}

async function fetchElo() {
	try {
		const r = await fetch('/elo', {cache: 'no-store'});
		eloPools = (await r.json()).pools || null;
	} catch (e) {
		eloPools = null; // no bridge, no claim
	}
	renderElo();
}
...
function handle(type, f) {
	...
	case MT.GAME_COMPLETE: {
		...
		fetchElo();        // and so did the ratings (rule 23e)
		break;
	}
	case MT.ERROR: {
		...
		log('Server: ' + msg);
		if (rejoinWait) {
			// An auto re-Join was refused: the dead chair may need one
			// more ask while the probe frees it (rule 3; KNOWN LIMITS) —
			// bounded, then given up out loud, never silently unseated.
			if (rejoinsLeft > 0) {
				clearTimeout(rejoinTimer);
				rejoinTimer = setTimeout(sendJoin, 1500);
			} else {
				rejoinWait = false;
				log('The seat would not come back — Join by hand.');
			}
		}
		if (pendingSend) snapBack();
		break;
...
// ---------------------------------------------------------------- connection
function linkWord() {
	// The wire's one word (rule 25): never a live-looking page over a dead
	// socket. Offline is the browser's own verdict; reconnecting is ours.
...
function paintLink() {
	const state = linkWord();
	$('connstate').classList.toggle('on', state === 'connected');
	$('connstate').classList.toggle('reconn', state === 'reconnecting');
	const word = $('connword');
	if (word) {
		word.textContent = state === 'reconnecting' ? 'reconnecting…' : state;
		word.className = 'connword' + (state ? ' ' + state : '');
	}
	// While the wire is down the board is a photograph, and it looks it.
	const bw = $('boardwrap');
	if (bw) bw.classList.toggle('stale', !!state && state !== 'connected');
}
...
function connect() {
	// The user's own click arms rule 25: from here, the page keeps the wire.
	wantLive = true;
	reconnDelay = 0;
...
function openSocket() {
	...
	try {
		...
		// A malformed address needs a human, not a retry loop.
	...
	ws.onopen = () => {
		sock.wasOpen = true;
		log('Connected.');
		reconnDelay = 0; // a wire that landed earns a fresh backoff
		clearTimeout(reconnTimer);
		reconnTimer = null;
		setConnected(true);
	...
	ws.onclose = () => {
		if (sock.wasOpen) log('Disconnected.');
		ws = null;
		setConnected(false);
		if (wantLive) scheduleReconnect();
	};
	ws.onerror = () => { if (sock.wasOpen) log('Connection trouble.'); };
}

function scheduleReconnect() {
	if (reconnTimer) return;
	reconnDelay = reconnDelay
		? Math.min(reconnDelay * 2, RECONN_CAP_MS) : RECONN_BASE_MS;
	// Half the step is certain, the rest is the jitter.
	const wait = reconnDelay / 2 + Math.random() * (reconnDelay / 2);
	reconnTimer = setTimeout(() => {
		reconnTimer = null;
		if (!wantLive || (ws && ws.readyState <= 1)) return;
		// A hidden page holds its hand; visibility's return redials.
		if (document.hidden) { paintLink(); return; }
		openSocket();
	}, wait);
}

function reconnectNow() {
	// Fresh news — the network came back, the page is being looked at —
	// resets the backoff and redials at once (rule 25).
	if (!wantLive || (ws && ws.readyState <= 1)) return;
	clearTimeout(reconnTimer);
	reconnTimer = null;
	reconnDelay = 0;
...
function resumeRole() {
	// Rule 25's resync: the old role is resumed uninvited, and rule 3's
	// join sync (Team + replay) rebuilds the board from the store — a move
	// recorded during the outage is on the board the moment the socket is.
	if (!reconnRole) return;
	rejoinsLeft = REJOIN_TRIES;
	sendJoin();
}

function sendJoin() {
	rejoinsLeft--;
	if (reconnRole === 'player') {
		rejoinWait = true;
		sendMsg(MT.JOIN, enc([[1, 1]]));
	} else {
		sendMsg(MT.JOIN, []);
	}
}
...
function clipKey(game, n) {
	return game + '#' + n;
}

function buildClipQueue() {
	// Policy in, mechanics out (rule 24g): the shared module owns arrival
	// order, one-at-a-time, exactly-once, the terminal dead state, the
	// element-event started truth, the progress-rearmed never-started
	// bound, and the advance; this page owns the fetch, the witness, and
	// the toggle. No phone anywhere in it — the module is neutral, and
	// the bridge serves it itself.
	return BrowserVoiceQueue.createVoiceQueue({
		startGraceMs: CLIP_START_GRACE_MS,
		deadKey: c => clipKey(c.game, c.n),
		// The speak toggle, consulted when a clip is dequeued: off means
		// the whole backlog goes silently (rule 24e).
		shouldPlay: chatSpeakOn,
		play: (c, clip) => {
			// The bridge synthesizes the RECORDED message through the
			// same piper pipeline every other voice of hers comes from
			// (rule 24e). A clip that cannot be fetched, decoded, or
			// started keeps the text, says so in the console, and the
			// queue moves on — one dead clip never parks the thread's
			// voice; a clip already sounding is never cut.
			fetch('/chat/audio?game=' + encodeURIComponent(c.game || '')
					+ '&n=' + c.n, {cache: 'no-store'})
				.then(r => {
				...
				.then(b => {
					...
					c.el = el;
					c.url = url;
					el.addEventListener('playing', () => clip.playing());
					el.addEventListener('timeupdate',
						() => clip.timeupdate(el.currentTime));
					el.addEventListener('progress', () => clip.progress());
					el.onended = () => clip.ended();
					el.onerror = () => clip.error('media error');
					el.play().catch(e => clip.rejected(e));
				})
				.catch(e => clip.failed((e && e.message) || String(e)));
		},
		unload: (c, outcome) => {
			// Every terminal outcome lands here once: a clip given up on
			// is silenced, and the object URL never leaks.
			if (outcome !== 'completed' && c.el) {
				try { c.el.pause(); } catch (e) { /* already dead */ }
			}
			if (c.url) URL.revokeObjectURL(c.url);
		},
		onFailed: (c, why) => {
			// Dead for good (rule 24e; phone.md rule 44a is the
			// pedigree): never refetched, never retried — the text keeps
			// the words. The browser's built-in narrator is NEVER
			// reached for: a generic voice wearing her words is worse
			// than silence, so the console line is the witness.
			log('Her voice is unavailable (' + why + ') — text only.');
		},
	});
}
const clipQ = buildClipQueue();

function dropClipQueue() {
	// Silence chosen is silence now: the clip already sounding plays out,
	// but nothing queued behind it will (rule 24e). The toggle going off,
	// a game switch, and a thread reset all land here.
	clipQ.dropQueued();
}

function enqueueClip(game, n) {
	// Her message `n` on the record, headed for the ONE clip queue (rule
	// 24e): in arrival order, one at a time, exactly once — never a later
	// message dropped because an earlier clip still plays, which is what
	// the first shipped shape ("one clip per batch is plenty") did. A
	// dead index is skipped at the door (the module's rule-44a state).
	clipQ.enqueue({game, n});
}
...
function chatReset() {
	...
	// A new thread starts from nothing: the open turn, the seed and any
	// pending echoes all belonged to the cleared log.
	chatTurn = null;
	chatSeed = null;
	chatEchoes = [];
	// A reset is a new thread: whatever waited to sound belonged to the
	// old one, and the refetched backlog is history, rendered silently.
	dropClipQueue();
}

function chatFold(s) {
	// The record folds every control and line-break character to a space at
	// the door (rule 24f); folding whitespace runs here lets an echo match
	// the record's copy of the same words without re-implementing that.
...
function chatContainer() {
	// History renders into the dimmed seed — the phone's own presentation of
	// what was already said when this page loaded; live turns land after it.
	const logEl = $('chat-log');
	if (!chatHistory) return logEl;
	if (!chatSeed) {
		chatSeed = document.createElement('div');
		chatSeed.id = 'chat-seed';
		chatSeed.appendChild(chatLine('seed-label', 'earlier at this table'));
		logEl.appendChild(chatSeed);
	}
	return chatSeed;
}

function adoptEcho(text) {
	// The record's copy of a message this page already echoed at the send:
	// the oldest pending echo with the same folded words is adopted in
	// place — the pending dim lifts, and her reply may join the turn it
	// opened. No match means the message was typed at another window.
	const folded = chatFold(text);
	for (let i = 0; i < chatEchoes.length; i++) {
		if (chatEchoes[i].text !== folded) continue;
		const el = chatEchoes[i].el;
		chatEchoes.splice(i, 1);
		el.className = 'turn';
		return el;
	}
	return null;
...
function renderChatMsg(m) {
	// The phone conversation's vocabulary (THE SHIPPED CLIENT bullet): the
	// sitter's line opens a turn as the dim `› ` you-line, her next message
	// joins the turn it answers, and a further — or unprompted — message of
	// hers stands as its own block. No name labels ride the lines: the
	// pane's head says who is at the table.
	const hers = m.who === 'assistant';
	const logEl = $('chat-log');
	if (!hers) {
		const adopted = adoptEcho(m.text);
		if (adopted) {
			chatTurn = {el: adopted, answered: false, seed: chatHistory};
		} else {
			const t = document.createElement('div');
			t.className = 'turn';
			t.appendChild(chatLine('chat-you', m.text));
			chatContainer().appendChild(t);
			chatTurn = {el: t, answered: false, seed: chatHistory};
		}
	} else if (chatTurn && !chatTurn.answered && chatTurn.seed === chatHistory) {
		// Her answer joins the turn that asked — but never across the
		// seed's dashed rule: a live reply must not render dimmed inside
		// history, nor history inside the live thread.
		chatTurn.el.appendChild(chatLine('chat-reply', m.text));
		chatTurn.answered = true;
	} else {
		const t = document.createElement('div');
		t.className = 'turn';
		t.appendChild(chatLine('chat-reply', m.text));
		chatContainer().appendChild(t);
		chatTurn = null;
	}
...
async function pollChat() {
	...
	for (let i = 0; i < batch.length; i++) {
		...
		// Every message of hers that ARRIVES while the toggle is on gets
		// its clip, exactly once, through the one queue (rule 24e).
		// chatCount still holds the ?since this batch was fetched with,
		// so since+i is the message's index on the record.
		if (hers && !chatHistory && chatSpeakOn())
			enqueueClip(chatGame, chatCount + i);
	}
	chatCount = j.count;
	chatHistory = false;
	updateChatChrome();
...
function dropEcho(echo, input, text) {
	// The record refused or never heard it: the echo comes off the screen
	// and the words go back in the box — never a message left on screen
	// that the record refused, and never typed words lost to a failure.
	const i = chatEchoes.indexOf(echo);
	if (i >= 0) chatEchoes.splice(i, 1);
	if (echo.el && echo.el.remove) echo.el.remove();
...
async function sendChat(e) {
	...
	if (!text) return;
	// The send is instant (THE SHIPPED CLIENT bullet): the words appear at
	// once as a pending turn, the box clears for the next thought, and the
	// record's own copy adopts the echo on the next poll. There is no
	// awaiting-reply placeholder on purpose — silence is an answer she may
	// choose (rule 24c's PASS), and a `…` that never resolves would be a
	// promise the table cannot keep.
	const t = document.createElement('div');
	t.className = 'turn pending';
	t.appendChild(chatLine('chat-you', text));
	const logEl = $('chat-log');
	logEl.appendChild(t);
	logEl.scrollTop = logEl.scrollHeight;
	const echo = {el: t, text: chatFold(text)};
	chatEchoes.push(echo);
	input.value = '';
	let r, j;
	try {
...
function paintThinking() {
	...
	el.textContent = txt || '';
	el.hidden = !txt;
	// The /thinking truth is the table's own activity: attentive while
	// called or genuinely considering, rest after the move lands.
	faceHint(!!(s.started_at || s.poked_at),
		s.stalled ? 'move stalled'
		: s.started_at ? 'considering the position'
		: s.poked_at ? 'called to the board'
		: 'at the table');
}
...
// ---------------------------------------------------------------- face
// Her portrait beside the table (specs/face.md, 2026-08-30 amendment).
// The shared FaceCard module carries the mechanics — manifest frames, the
// broker's automatic and explicit expression, real-clip mouth motion, and
// the remembered three-step size. THIS file keeps the table's policy: the
// /thinking truth stays the local activity hint when the broker has
// nothing stronger, the caption stays the exact status account, and
// expression is never derived from evaluation.
let faceCard = null, faceCaption = 'at the table', faceSpeaking = false;

function faceHint(attentive, caption) {
	faceCaption = caption;
	if (faceCard) faceCard.setHint(attentive ? 'attentive' : null);
	paintFaceCaption();
}

function paintFaceCaption() {
	const stateEl = $('portrait-state');
	if (stateEl) stateEl.textContent = faceCaption
		+ (faceSpeaking ? ' · speaking' : '');
}

function faceBoot() {
	if (!window.FaceCard) return;
	const stack = document.querySelector('.portrait-stack');
	if (!stack) return;
	faceCard = FaceCard.init({
		root: stack,
		routes: {
			manifest: '/face/manifest.json',
			state: '/face/state',
			asset: id => '/face/asset/' + encodeURIComponent(id),
		},
		fetchOpts: {cache: 'no-store'},
		sizeKey: 'deskcrab-face-size-chess',
		sizes: ['compact', 'medium', 'large'],
		defaultSize: 'medium',
		sizeButton: $('face-size'),
		onState(st) {
			faceSpeaking = !!(st && st.speaking);
			paintFaceCaption();
		},
	});
}
...
window.addEventListener('DOMContentLoaded', () => {
	...
	// The chosen role is remembered so a redial can resume it (rule 25).
	$('join').onclick = () => {
		reconnRole = 'player';
		sendMsg(MT.JOIN, enc([[1, 1]]));
	};
	...
	// Rule 25: fresh news redials at once; a hidden page holds its hand
	// until it is looked at again; going offline repaints the word.
	...
	$('chat-speak').addEventListener('change', () => {
		// Off empties the queue (rule 24e): silence chosen is silence
		// now, not after the backlog plays out.
		if (!$('chat-speak').checked) dropClipQueue();
		try {
