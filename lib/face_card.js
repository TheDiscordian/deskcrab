...
(function () {
  function init(opts) {
    ...
    const EXPR_MOTION = {
      ...
      // The one-shot shake carries the scold; the held pose must not buzz
      // through a run of ordinary failed game clicks.
      annoyed: { energy: 0.6, agitation: 0.18, droop: 0 },
      startled: { energy: 0.8, agitation: 0.45, droop: 0 },
      tired: { energy: 0.35, agitation: 0, droop: 0.7 },
      sleeping: { energy: 0.12, agitation: 0, droop: 0.9 },
    };
    const EXPR_GESTURE = { annoyed: "shake", pleased: "bounce",
                           startled: "perk" };
    function motionForExpression(name) {
      if (engine.mappedExpr === name) return;
      const previous = engine.mappedExpr;
      engine.mappedExpr = name;
      if (engine.override) return;   // her hand outranks the mapping
      const motion = EXPR_MOTION[name] || EXPR_MOTION.resting;
      setChannels(motion);
      const previousMotion = EXPR_MOTION[previous] || EXPR_MOTION.resting;
      // Do not let the fast scold driver ring into a calm expression. Root
      // sway and breathing continue; only agitation and its spring kick end.
      if (previous && previousMotion.agitation && !motion.agitation) {
        engine.channels.agitation = 0;
        if (engine.gesture && engine.gesture.name === "shake")
          engine.gesture = null;
        for (const region of engine.regions) region.v = 0;
      }
    ...
    let mouthHideTimer = null, mouthMorphSeq = 0;
    function buildMouth() {
      const m = card.manifest;
      ...
      // One painted patch, always.  Two opacity-crossfaded slots made both
      // contours coexist and exposed the expression plate's resting mouth.
      for (let i = 0; i < 1; i++) {
    ...
    // The contour interpolation of rule 24: one opaque patch changes source
    // and eases geometrically from the outgoing extent to the incoming one.
    // There is never an old+new opacity blend, hence never two mouth lines.
    function morphScale(from, to) {
      const a = visExtent(from), b = visExtent(to);
      if (!a || !b) return null;
      // Never enlarge the incoming skin patch beyond its authored bounds.
      // A tall outgoing contour followed by a short one used to stretch the
      // whole 50x34 patch up to 2x, putting its rectangular edges over the
      // nose and chin.  Smaller contours may still grow smoothly into larger
      // ones; the reverse transition swaps at the smaller authored size.
      return "scale(" + clamp(a[0] / b[0], 0.6, 1.0).toFixed(3) + ","
        + clamp(a[1] / b[1], 0.5, 1.0).toFixed(3) + ")";
    }
    function showViseme(vis) {
      ...
      const seq = ++mouthMorphSeq;
      if (mouthHideTimer) {
        clearTimeout(mouthHideTimer);
        mouthHideTimer = null;
      }
      if (vis === "rest" || !spec) {
        if (mouth && mouth.dataset.vis && !reduced) {
          mouth.style.display = "block";
          mouth.style.opacity = "1";
          mouth.style.transition = "transform 100ms ease-out";
          const toRest = morphScale("rest", mouth.dataset.vis);
          mouth.style.transform = toRest ? toRest : "";
          mouthHideTimer = setTimeout(() => {
            if (seq === mouthMorphSeq) {
      ...
      if (reduced) {
        ...
        // Commit the new contour at the old contour's dimensions, fully
        // opaque, then ease that one contour to its authored dimensions.
        incoming.style.transition = "none";
        incoming.style.opacity = "1";
        const start = morphScale(from || "rest", vis);
        incoming.style.transform = start ? start : "";
        void incoming.offsetWidth;
        incoming.style.transition = "transform 100ms ease-out";
        incoming.style.transform = "scale(1,1)";
      }
      activeMouth = next;
    }
