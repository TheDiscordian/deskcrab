...
11. The engineering drawer reaches the speaking prompts (turn and wake) through `lib/eng prompt`,
    ...
    the shelf pattern, not the archive: the full rendering shows the freshest outcomes
    (`DESKCRAB_ENG_SETTLED_RECENT`, default 10) with the older tail as a count naming the drawer
    and `crab eng list --state settled` as the way in; `--compact` (the interactive turn profile)
...
body prose stays out of the prompt, the settled tail is the freshest few plus the count line
naming the drawer, `--compact` is the count line alone — and migration preserving dates and
idempotence);
