...
deep_note() {
    printf '## tonight\nthe write-up\n' > "$DEEP/tonight.md"
    touch -d "@$(( NOW - 60 ))" "$DEEP/tonight.md"
}
...
echo
echo "with the writing root named, the same claim is backed by its own file:"
reset; poison; rm -f "$DEEP"/*.md; deep_note
"$T/repo/lib/promise-check" turn wake "$NOW" 5041 "$(snap)" \
    "$T/ledger.jsonl" "$DEEP_CLAIM" >/dev/null 2>&1
check_eq "the model was never called" "$(claude_n)" "0"
check_eq "nothing ledgered, nothing booked" "$(ledger_n)$(records)" "00"
check "the trace names the resolved artefact" \
...
echo
echo "the extra root loosens nothing — an unwritten file under it still flags:"
reset; poison; rm -f "$DEEP"/*.md
"$T/repo/lib/promise-check" turn wake "$NOW" 5042 "$(snap)" \
    "$T/ledger.jsonl" "$DEEP_CLAIM" >/dev/null 2>&1
check_eq "the model WAS consulted" "$(claude_n)" "1"
check_eq "and the flag lands: one ledger line, one wake" "$(ledger_n)$(records)" "11"

echo
echo "a root that does not exist is skipped in silence:"
reset; poison; rm -f "$DEEP"/*.md; deep_note
echo "PROMISE_PATH_ROOTS='$T/nowhere:$HOUSE/Library'" >> "$DESKCRAB_CONF"
"$T/repo/lib/promise-check" turn wake "$NOW" 5043 "$(snap)" \
    "$T/ledger.jsonl" "$DEEP_CLAIM" >/dev/null 2>&1
check_eq "the good root behind it still answers" "$(claude_n)" "0"
check_eq "nothing ledgered, nothing booked" "$(ledger_n)$(records)" "00"
