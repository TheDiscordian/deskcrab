...
2. Mouth normalisation (rule 55). The 2026-08-30 `open` and `wide` patches
   were dramatically larger than the rest of the family and lurched when
   swapped. Each oversized patch's changed region (diff against the plate's
   own mouth rect) is scaled down about the family's lip anchor and
   re-composited over stable plate skin, so the border ring stays exact
   plate pixels (rule 9). Originals are preserved, measurements recorded.
...
def normalise_mouth(viseme, scale, plate_rect, src_path, dst_path):
    ...
    def rescale(img, resample):
        ...
        ox = int(round(ax * SS - ax * SS * scale))
...
def main():
    ...
    for v, scale in MOUTH_SCALE.items():
        ...
        before, after = normalise_mouth(
            v, scale, plate_rect, PRESERVED / f"mouth-{v}.png", dst)
        manifest["assets"][f"mouth-{v}"]["sha256"] = sha(dst)
        manifest["visemes"][v]["extent"] = after
        record["mouth"]["extents"][v] = {"before": before, "after": after}
